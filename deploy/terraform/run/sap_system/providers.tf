  ...
  */
  ...
  provider "azurerm"                     {
                                          features {}
                                          alias                      = "dnsmanagement"
                                          subscription_id            = coalesce(var.management_dns_subscription_id, try(data.azurerm_key_vault_secret.cp_subscription_id[0].value,""), local.SAPLibrary_subscription_id)
                                          client_id                  = var.use_spn ? data.azurerm_key_vault_secret.cp_client_id[0].value : null
                                          client_secret              = var.use_spn ? ephemeral.azurerm_key_vault_secret.cp_client_secret[0].value : null
                                          tenant_id                  = var.use_spn ? data.azurerm_key_vault_secret.cp_tenant_id[0].value : null
                                          use_msi                    = var.use_spn ? false : true
  ...
  provider "azurerm"                     {
                                          ...
                                          subscription_id            = coalesce(var.privatelink_dns_subscription_id, var.management_dns_subscription_id, try(data.azurerm_key_vault_secret.cp_subscription_id[0].value,""), local.SAPLibrary_subscription_id)
                                          client_id                  = var.use_spn ? data.azurerm_key_vault_secret.cp_client_id[0].value : null
                                          client_secret              = var.use_spn ? ephemeral.azurerm_key_vault_secret.cp_client_secret[0].value : null
                                          tenant_id                  = var.use_spn ? data.azurerm_key_vault_secret.cp_tenant_id[0].value : null
                                          use_msi                    = var.use_spn ? false : true
