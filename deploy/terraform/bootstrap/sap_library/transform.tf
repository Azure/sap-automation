...
locals {
  ...
  storage_account_sapbits = {
    ...
    account_replication_type = coalesce(var.library_sapmedia_account_replication_type, try(var.storage_account_sapbits.account_replication_type, "ZRS"))
    ...
    file_share = {
      ...
      name              = coalesce(var.library_sapmedia_file_share_name, try(var.storage_account_sapbits.file_share.name, module.sap_namegenerator.naming.resource_suffixes.sapbits))
    }
    sapbits_blob_container = {
      ...
      name              = coalesce(var.library_sapmedia_blob_container_name, try(var.storage_account_sapbits.sapbits_blob_container.name, module.sap_namegenerator.naming.resource_suffixes.sapbits))
    }
  }
  storage_account_tfstate = {
    ...
    account_replication_type = coalesce(var.library_terraform_state_account_replication_type, try(var.storage_account_tfstate.account_replication_type, "ZRS"))
