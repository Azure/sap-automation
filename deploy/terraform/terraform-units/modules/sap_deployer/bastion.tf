...
resource "azurerm_public_ip" "bastion" {
  ...
  name  = format("%s%s%s", local.resource_prefixes.bastion_pip, local.prefix, local.resource_suffixes.bastion_pip)
...
resource "azurerm_bastion_host" "bastion" {
  count = var.bastion_deployment ? 1 : 0
  name  = format("%s%s%s", local.resource_prefixes.bastion_host, local.prefix, local.resource_suffixes.bastion_host)
  location = local.vnet_mgmt_exists ? (
    data.azurerm_virtual_network.vnet_mgmt[0].location) : (
    azurerm_virtual_network.vnet_mgmt[0].location
  )
