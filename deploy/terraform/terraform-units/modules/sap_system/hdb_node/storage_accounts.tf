...
resource "azurerm_storage_account" "hanashared" {
  ...
  name                                 = substr(replace(
                                           lower(
                                             format("%s%s%01d%s",
                                               local.prefix,
                                               local.resource_suffixes.hanasharedafs, count.index,substr(random_id.random_id.hex, 0, 2)
                                             )
                                           ),
                                           "/[^a-z0-9]/",
                                           ""
...
resource "azurerm_storage_share" "hanashared" {
 ...
 depends_on                           = [
  ...
  storage_account_name                 = var.NFS_provider == "AFS" ? (
                                           length(var.azure_files_hanashared_id{count.index]) > 0 ? (
...
resource "azurerm_private_endpoint" "hanashared" {
  ...
  private_service_connection {
                              ...
                              private_connection_resource_id = length(var.hanashared_private_endpoint_id[count.index]) > 0 ? (
                                var.hanashared_private_endpoint_id[count.index]) : (
