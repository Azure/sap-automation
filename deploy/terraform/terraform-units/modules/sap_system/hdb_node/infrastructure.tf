...
# TODO:
# Current behavior, it will try to add all VMs in the cluster into the
# backend pool, which would not work since we do not have availability
# sets created yet.
# In a scale-out scenario, we need to rewrite this code according to the
# scale-out + HA reference architecture.
resource "azurerm_network_interface_backend_address_pool_association" "hdb" {
  ...
  network_interface_id                 = local.db_lb_network_interface_ids[count.index]
  ip_configuration_name                = local.db_lb_ip_configuration_names[count.index]
...
resource "azurerm_private_dns_a_record" "db" {
  ...
  name                                 = lower(coalesce(
                                           try(var.infrastructure.configuration_settings.custom_db_virtual_hostname,""),
                                           format("%s%sdb%scl", var.sap_sid, local.database_sid, local.database_instance))
                                         )
