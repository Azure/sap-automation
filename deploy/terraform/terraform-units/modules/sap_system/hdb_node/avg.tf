...
resource "azurerm_netapp_volume_group_sap_hana" "avg_HANA" {
  ...
  count    = var.NFS_provider == "ANF" && local.use_avg && var.deployment == "new" && length(var.scale_set_id) == 0 ? length(var.ppg) : 0
...
locals {
  ...
  hana_data1 = {
    ...
    volumeSpecName          = "data"
    proximityPlacementGroup = length(var.scale_set_id) == 0 ? var.ppg[0] : null
  ...
  hana_data2 = {
    ...
    volumeSpecName          = "data"
    proximityPlacementGroup = length(var.scale_set_id) == 0 ? (length(var.ppg) > 1 ? var.ppg[1] : var.ppg[0]) : null
    storage_quota_in_gb     = var.hana_ANF_volumes.data_volume_size
    throughput_in_mibps     = var.hana_ANF_volumes.data_volume_throughput
