...
resource "azurerm_netapp_volume_group_sap_hana" "avg_HANA_data1" {
  provider                             = azurerm.main
  count                                = local.use_avg  ? length(var.database.zones) * (var.database_server_count - var.database.stand_by_node_count - 1) : 0
