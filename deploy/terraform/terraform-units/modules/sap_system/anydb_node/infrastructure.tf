...
resource "azurerm_lb" "anydb" {
  ...
  name     = format("%s%s%s%s", local.resource_prefixes.db_alb, local.prefix, var.naming.separator, local.resource_suffixes.db_alb)
  sku      = "Standard"
  ...
  frontend_ip_configuration {
    name      = format("%s%s%s%s", local.resource_prefixes.db_alb_feip, local.prefix, var.naming.separator, local.resource_suffixes.db_alb_feip)
...
resource "azurerm_lb_backend_address_pool" "anydb" {
  count           = local.enable_db_lb_deployment ? 1 : 0
  name            = format("%s%s%s%s", local.resource_prefixes.db_alb_bepool, local.prefix, var.naming.separator, local.resource_suffixes.db_alb_bepool)
...
resource "azurerm_lb_probe" "anydb" {
  provider            = azurerm.main
  count               = local.enable_db_lb_deployment ? 1 : 0
  name                = format("%s%s%s%s", local.resource_prefixes.db_alb_hp, local.prefix, var.naming.separator, local.resource_suffixes.db_alb_hp)
  loadbalancer_id     = azurerm_lb.anydb[count.index].id
...
resource "azurerm_lb_rule" "anydb" {
  ...
  name                           = format("%s%s%s%s", local.resource_prefixes.db_alb_rule, local.prefix, var.naming.separator, local.resource_suffixes.db_alb_rule)
  protocol                       = "All"
  frontend_port                  = 0
  backend_port                   = 0
  frontend_ip_configuration_name = format("%s%s%s%s", local.resource_prefixes.db_alb_feip, local.prefix, var.naming.separator, local.resource_suffixes.db_alb_feip)
