...
resource "azurerm_network_interface" "observer" {
  ...
  name                          = format("%s%s%s%s%s", local.resource_prefixes.nic, local.prefix, var.naming.separator, var.naming.virtualmachine_names.OBSERVER_VMNAME[count.index], local.resource_suffixes.nic)
...
resource "azurerm_linux_virtual_machine" "observer" {
  ...
  resource_group_name = var.resource_group[0].name
  location            = var.resource_group[0].location
  name                = format("%s%s%s%s%s", local.resource_prefixes.vm, local.prefix, var.naming.separator, var.naming.virtualmachine_names.OBSERVER_VMNAME[count.index], local.resource_suffixes.vm)
  ...
  os_disk {
    name                   = format("%s%s%s%s%s", local.resource_prefixes.osdisk, local.prefix, var.naming.separator, var.naming.virtualmachine_names.OBSERVER_VMNAME[count.index], local.resource_suffixes.osdisk)
    caching                = "ReadWrite"
