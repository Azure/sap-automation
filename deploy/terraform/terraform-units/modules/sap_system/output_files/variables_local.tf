...
locals {
  ...
  iscsi_scs_servers    = var.scs_cluster_type == "ISCSI" ? distinct(flatten([for idx, vm in var.iSCSI_servers : [
                          format("{ iscsi_host: '%s', iqn: %s, type ='scs' }", vm.host, format("iqn.2006-04.ascs%s.local:ascs%s", var.sap_sid, var.sap_sid))]
                          ])) : []
  iscsi_db_servers     = var.database_cluster_type == "ISCSI" ? distinct(flatten([for idx, vm in var.iSCSI_servers : [
