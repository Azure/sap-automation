resource "azurerm_private_dns_a_record" "app_secondary" {
  count               = var.use_secondary_ips && !var.use_custom_dns_a_registration && length(var.dns) > 0 ? length(var.application_server_secondary_ips) : 0
...
resource "azurerm_private_dns_a_record" "scs_secondary" {
  count               = var.use_secondary_ips && !var.use_custom_dns_a_registration && length(var.dns) > 0 ? length(var.scs_server_secondary_ips) : 0
...
resource "azurerm_private_dns_a_record" "web_secondary" {
  count               = var.use_secondary_ips && !var.use_custom_dns_a_registration && length(var.dns) > 0 ? length(var.webdispatcher_server_secondary_ips) : 0
