...
output "sapmnt_path" {
  value = var.NFS_provider == "AFS" ? (
    format("%s:/%s/%s", split("/", replace(azurerm_storage_share.sapmnt[0].url, "https://", ""))[0], azurerm_storage_account.sapmnt[0].name, azurerm_storage_share.sapmnt[0].name)
    ) : (
...
output "install_path" {
  value = var.NFS_provider == "AFS" ? try(format("%s:/%s/%s", split("/", replace(azurerm_storage_share.install[0].url, "https://", ""))[0], azurerm_storage_account.install[0].name, azurerm_storage_share.install[0].name), "") : ""
}
