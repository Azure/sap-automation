...
resource "azurerm_network_interface" "app" {
  ...
  name                          = format("%s%s%s%s%s", local.resource_prefixes.nic, local.prefix, var.naming.separator, var.naming.virtualmachine_names.APP_VMNAME[count.index], local.resource_suffixes.nic)
...
resource "azurerm_network_interface" "app_admin" {
  ...
  name                          = format("%s%s%s%s%s", local.resource_prefixes.admin_nic, local.prefix, var.naming.separator, var.naming.virtualmachine_names.APP_VMNAME[count.index], local.resource_suffixes.admin_nic)
  location                      = var.resource_group[0].location
  resource_group_name           = var.resource_group[0].name
  enable_accelerated_networking = local.app_sizing.compute.accelerated_networking
...
resource "azurerm_linux_virtual_machine" "app" {
  ...
  name                = format("%s%s%s%s%s", local.resource_prefixes.vm, local.prefix, var.naming.separator, var.naming.virtualmachine_names.APP_VMNAME[count.index], local.resource_suffixes.vm)
  ...
  dynamic "os_disk" {
    ...
    content {
      name                   = format("%s%s%s%s%s", local.resource_prefixes.osdisk, local.prefix, var.naming.separator, var.naming.virtualmachine_names.APP_VMNAME[count.index], local.resource_suffixes.osdisk)
...
resource "azurerm_managed_disk" "app" {
  ...
  name                   = format("%s%s%s%s%s", local.resource_prefixes.disk, local.prefix, var.naming.separator, var.naming.virtualmachine_names.APP_VMNAME[local.app_data_disks[count.index].vm_index], local.app_data_disks[count.index].suffix)
  location               = var.resource_group[0].location
  resource_group_name    = var.resource_group[0].name
  create_option          = "Empty"
