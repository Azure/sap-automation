...
data "azurerm_network_security_group" "nsg_app" {
  provider            = azurerm.main
  count               = local.enable_deployment ? (local.application_subnet_nsg_exists ? 1 : 0) : 0
...
resource "azurerm_subnet_network_security_group_association" "Associate_nsg_app" {
  provider                  = azurerm.main
  count                     = local.enable_deployment ? (signum((local.application_subnet_exists ? 0 : 1) + (local.application_subnet_nsg_exists ? 0 : 1))) : 0
  subnet_id                 = local.application_subnet_exists ? local.application_subnet_arm_id : azurerm_subnet.subnet_sap_app[0].id
  network_security_group_id = local.application_subnet_nsg_exists ? data.azurerm_network_security_group.nsg_app[0].id : azurerm_network_security_group.nsg_app[0].id
}
