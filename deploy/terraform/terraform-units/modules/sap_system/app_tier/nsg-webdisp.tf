...
resource "azurerm_network_security_group" "nsg_web" {
  ...
  count               = local.enable_deployment && local.web_subnet_defined ? (local.web_subnet_nsg_exists ? 0 : 1) : 0
...
data "azurerm_network_security_group" "nsg_web" {
  provider            = azurerm.main
  count               = local.enable_deployment && local.web_subnet_defined ? (local.web_subnet_nsg_exists ? 1 : 0) : 0
...
resource "azurerm_subnet_network_security_group_association" "Associate_nsg_web" {
  ...
  provider                  = azurerm.main
  count                     = local.enable_deployment && local.web_subnet_defined ? (signum((local.web_subnet_exists ? 0 : 1) + (local.web_subnet_nsg_exists ? 0 : 1))) : 0
  subnet_id                 = local.web_subnet_exists ? data.azurerm_subnet.subnet_sap_web[0].id : azurerm_subnet.subnet_sap_web[0].id
