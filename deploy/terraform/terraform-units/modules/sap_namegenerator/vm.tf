locals {
  ...
  anydb_computer_names_ha = [for idx in range(var.db_server_count * 2) :
    format("%sdb%02d%s%01d%s", lower(var.sap_sid), floor(idx/2) + var.resource_offset, local.db_oscode, tonumber((( idx + 1 ) % 2)), local.random_id_vm_verified)
  ]
  ...
  anydb_vm_names_ha = [for idx in range(var.db_server_count * 2) :
    length(var.db_zones) > 0 && var.use_zonal_markers ? (
      format("%sdb%sz%s%s%02d%s%01d%s", lower(var.sap_sid), local.separator, local.ha_zones[idx % max(length(local.ha_zones), 1)], local.separator, floor(idx/2) + var.resource_offset, local.db_oscode, tonumber((( idx + 1 ) % 2)), local.random_id_vm_verified)) : (
      format("%sdb%02d%s%01d%s", lower(var.sap_sid), floor(idx/2) + var.resource_offset, local.db_oscode, tonumber((( idx + 1 ) % 2)), local.random_id_vm_verified)
    )
  ]
  ...
  hana_computer_names_ha = [for idx in range(var.db_server_count * 2) :
    format("%sd%s%02dl%01d%s", lower(var.sap_sid), lower(var.db_sid), floor(idx/2) + var.resource_offset + var.resource_offset, tonumber((( idx + 1 ) % 2)), substr(local.random_id_vm_verified, 0, 2))
  ]
  ...
  hana_server_vm_names_ha = [for idx in range(var.db_server_count * 2) :
    length(var.db_zones) > 0 && var.use_zonal_markers ? (
      format("%sd%s%sz%s%s%02dl%01d%s", lower(var.sap_sid), lower(var.db_sid), local.separator, local.ha_zones[idx % max(length(local.ha_zones), 1)], local.separator, floor(idx/2) + var.resource_offset, tonumber((( idx + 1 ) % 2)), local.random_id_vm_verified)) : (
      format("%sd%s%02dl%01d%s", lower(var.sap_sid), lower(var.db_sid), floor(idx/2) + var.resource_offset, tonumber((( idx + 1 ) % 2)), local.random_id_vm_verified)
    )
  ]
  ...
  anydb_secondary_dnsnames_ha = [for idx in range(var.db_server_count * 2) :
    format("v%sd%s%02dl%01d%s", lower(var.sap_sid), lower(var.db_sid), floor(idx/2) + var.resource_offset, tonumber((( idx + 1 ) % 2)), substr(local.random_id_vm_verified, 0, 2))
  ]
  ...
  hana_secondary_dnsnames_ha = [for idx in range(var.db_server_count * 2) :
    format("v%sd%s%02dl%01d%s", lower(var.sap_sid), lower(var.db_sid), floor(idx/2) + var.resource_offset, tonumber((( idx + 1 ) % 2)), local.random_id_virt_vm_verified)
  ]
