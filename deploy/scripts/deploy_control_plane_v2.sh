...
function bootstrap_deployer() {
    ...
    if [ 0 -eq "$step" ]; then
        ...
        echo "Calling install_deployer:         ${allParameters[*]}"

        if install_deployer "${allParameters[@]}"; then
            local_return_code=$?
...
function validate_keyvault_access {
    ...
    if ! printenv DEPLOYER_KEYVAULT; then
        ...
        load_config_vars "${deployer_environment_file_name}" "DEPLOYER_KEYVAULT" "APPLICATION_CONFIGURATION_NAME" "APPLICATION_CONFIGURATION_DEPLOYMENT"
        echo "${deployer_environment_file_name}"
        cat "${deployer_environment_file_name}"

        if [ "${APPLICATION_CONFIGURATION_DEPLOYMENT:-false}" == "true" ]; then
...
function bootstrap_library {
    ...
    if [ 2 -eq "$step" ]; then
        ...
        allParameters=(--parameter_file "${library_parameter_file}")
...
function retrieve_parameters() {
    ...
    if is_valid_id "${APPLICATION_CONFIGURATION_ID:-}" "/providers/Microsoft.AppConfiguration/configurationStores/"; then
        ...
        if [ -f "${deployer_dirname}/.terraform/terraform.tfstate" ]; then
            local_backend=$(grep "\"type\": \"azurerm\"" "${deployer_dirname}/.terraform/terraform.tfstate" || true)
...
function deploy_control_plane() {
    ...
    helper_scripts=(
        ...
        "${SAP_AUTOMATION_REPO_PATH}/deploy/scripts/install_deployer_v2.sh"
        "${SAP_AUTOMATION_REPO_PATH}/deploy/scripts/install_library_v2.sh"
        "${SAP_AUTOMATION_REPO_PATH}/deploy/scripts/installer_v2.sh"
    ...
    printf -v app_config_name '%-40s' "${APPLICATION_CONFIGURATION_NAME:-Undefined}"
    printf -v ctrl_plane_name '%-40s' "$CONTROL_PLANE_NAME"
...
| Application Configuration             | ${APPLICATION_CONFIGURATION_NAME:-NotUsed}   |
